...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      const stored = localStorage.getItem("gf_registered_user");
      let user;

      if (stored) {
        const registered = JSON.parse(stored);
        if (registered.email === form.email) {
          if (registered.password !== form.password) {
      ...
      if (!user) {
        // Allow any email/password combo as demo login
        user = {
          email: form.email,
      ...
      <main className="flex-grow flex items-center justify-center pt-16 sm:pt-20 md:pt-24 pb-12 px-4 relative overflow-hidden">
        ...
        >
          ...
          <div className="mt-8 text-center text-sm text-muted-foreground">
            ...
            <Link to="/register" className="text-primary font-bold hover:underline">Start building for free</Link>
          </div>
        </motion.div>
      </main>
